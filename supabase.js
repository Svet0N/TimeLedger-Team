/* ================================================================
   supabase.js — ShiftFlow 2.0  Data layer + SQL schema
   ================================================================ */
...
function initDemoData() {
  if (localStorage.getItem('sf_v3_init')) return;

  const users = [
    { id:'mgr-1', username:'manager1', password:'1234', role:'manager',  name:'Главен Мениджър', hourly_rate:0,   status:'active' },
    { id:'emp-1', username:'emp1',     password:'1234', role:'employee', name:'Иван Петров',     hourly_rate:9,   status:'active' },
    { id:'emp-2', username:'emp2',     password:'1234', role:'employee', name:'Мария Иванова',   hourly_rate:8.5, status:'active' },
    { id:'emp-3', username:'emp3',     password:'1234', role:'employee', name:'Георги Стоев',    hourly_rate:10,  status:'active' },
    { id:'emp-4', username:'emp4',     password:'1234', role:'employee', name:'Елена Николова',  hourly_rate:9.5, status:'active' },
  ];
  ...
  localStorage.setItem('sf_v3_init', '1');
}
...
const DB = {
  // Users
  getEmployees() {
    return JSON.parse(localStorage.getItem('sf_users_v3')||'[]').filter(u=>u.role==='employee');
  },
  getEmployee(id) {
  ...
  // Notifications
  getNotifications() {
    return JSON.parse(localStorage.getItem('sf_notifs_v3')||'[]').sort((a,b)=>b.date.localeCompare(a.date));
  },
  ...
  markAllNotifsRead(userId) {
    const arr = JSON.parse(localStorage.getItem('sf_notifs_v3')||'[]');
    arr.forEach(n => { if(!n.read_by.includes(userId)) n.read_by.push(userId); });
    localStorage.setItem('sf_notifs_v3', JSON.stringify(arr));
  },
  getUnreadCount(userId) {
    return this.getNotifications().filter(n=>!n.read_by.includes(userId)).length;
  },
};
