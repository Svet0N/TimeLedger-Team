...
function buildExportData() {
  ...
  DB.getEmployees().forEach(emp2=>{
    DB.getShifts(emp2.id,from,to).forEach(s=>{
      ...
      rows.push({'Служител':emp2.name,'Дата':s.date,'Часове':s.worked_hours,'Check-in':s.checkin_time||'',
...
function exportCSV() {
  ...
  dlFile(csv,`ShiftFlow_${mgr.reportYear}-${String(mgr.reportMonth+1).padStart(2,'0')}.csv`,'text/csv;charset=utf-8;');
...
function exportExcel() {
  ...
  dlFile(html,`ShiftFlow_${mgr.reportYear}-${String(mgr.reportMonth+1).padStart(2,'0')}.xls`,'application/vnd.ms-excel');
...
function exportPDF() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>ShiftFlow — ${ml}</title>
  ...
  </head><body><h1>⏱ ShiftFlow — Отчет</h1><div class="sub">${ml} · ${new Date().toLocaleDateString('bg-BG')}</div>
